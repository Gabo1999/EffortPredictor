		...
		.then(response => {
            if (response.ok) {
				...
				response.json().then(function(text) {
					responseSomething = text['Profile'];
					console.log(typeof(responseSomething))
	...
	<main class="flex-grow">
		{#if responseSomething}
			<div class="text-center mb-10">
				<h1 class="sm:text-3xl text-2xl font-medium text-center title-font text-gray-900 mb-4">Account</h1>
			</div>
			<div class="flex flex-col text-center mt-14 items-center justify-center">
				<img
				src="https://mdbcdn.b-cdn.net/img/new/avatars/8.webp"
				class="rounded-full w-44 mb-4 mx-auto"
				alt="Avatar"
				/>
				<h5 class="text-xl font-medium leading-tight mb-2">{responseSomething.first_name} {responseSomething.last_name}</h5>
				<p class="text-gray-500">{responseSomething.username}</p>
				<div class="w-12 h-1 bg-cemex-red rounded mt-2 mb-4"></div>
			</div>
			<div class="bg-white mx-36 flex flex-wrap py-6 rounded shadow-md text-center items-center justify-center content-center">
				<div class="lg:w-1/2 px-6">
				<h2 class="title-font font-semibold text-gray-900 tracking-widest text-xs">DATE JOINED</h2>
				<p>{responseSomething.date_joined}</p>
				</div>
				<div class="lg:w-1/2 px-6 lg:mt-0">
				<h2 class="title-font font-semibold text-gray-900 tracking-widest text-xs">EMAIL</h2>
				<!-- svelte-ignore a11y-missing-attribute -->
				<a class="text-cemex-red leading-relaxed">{responseSomething.email}</a>
				</div>
			</div>
			<div class="items-center text-center">
				<button type="button" onclick="location.href='http://127.0.0.1:8000/accounts/password_change'" class="flex mx-auto mt-16 text-white bg-[#023185] border-0 py-2 px-8 focus:outline-none hover:bg-[#02215a] rounded text-lg">Change Password</button>
				<button type="button" onclick="location.href='http://127.0.0.1:8000/admin/'" class="flex mx-auto mt-8 text-white bg-[#023185] border-0 py-2 px-16 focus:outline-none hover:bg-[#02215a] rounded text-lg">Admin Site</button>
			</div>
		{/if}
	</main>
